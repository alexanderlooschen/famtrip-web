...
export default function ReiseNeuPage() {
  ...
  const etappenSpeichern = async () => {
    if (!reiseId) return;
    const ungueltig = etappen.find(e =>
      !e.titel || !e.ort || !e.datum_von || !e.datum_bis || e.verkehrsmittel.length === 0
    );
    if (ungueltig) { setFehler('Bitte alle Etappen vollständig ausfüllen.'); return; }
    setFehler(''); setLaden(true);
        ...
        <div className="space-y-5">
          <Feld label="Reise-Titel *">
            <input value={titel} onChange={e => setTitel(e.target.value)}
              placeholder="z. B. Radtour Osnabrücker Land 2026"
              className="eingabe" />
          </Feld>
          ...
          <Feld label="Gesamtfazit / Abschlusskommentar">
            <textarea value={gesamtkommentar} onChange={e => setGesamtkommentar(e.target.value)}
              rows={3} placeholder="Wie war die Reise insgesamt? Was würdet ihr anderen Familien empfehlen oder abraten?"
              className="eingabe resize-none" />
          </Feld>

          <div className="grid grid-cols-2 gap-4">
            <Feld label="Reisebeginn *">
              <input type="date" value={datumVon} onChange={e => setDatumVon(e.target.value)} className="eingabe" />
            </Feld>
            <Feld label="Reiseende *">
      ...
      {schritt === 'etappe' && (
        <div>
          {etappen.map((etappe, idx) => (
            <div key={idx} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 mb-5">
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-3">
                  <Feld label="Titel *">
                    <input value={etappe.titel}
                      onChange={e => etappeAktualisieren(idx,'titel',e.target.value)}
                      placeholder="z. B. Ankunft Osnabrück" className="eingabe" />
                  </Feld>
                  <Feld label="Ort *">
                    <input value={etappe.ort}
                      onChange={e => etappeAktualisieren(idx,'ort',e.target.value)}
                ...
                <div className="grid grid-cols-2 gap-3">
                  <Feld label="Von *">
                    <input type="date" value={etappe.datum_von}
                      onChange={e => etappeAktualisieren(idx,'datum_von',e.target.value)} className="eingabe" />
                  </Feld>
                  <Feld label="Bis *">
                    <input type="date" value={etappe.datum_bis}
                      onChange={e => etappeAktualisieren(idx,'datum_bis',e.target.value)} className="eingabe" />
                  </Feld>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <Feld label="Unterkunft-Typ">
                    <select value={etappe.unterkunft_typ}
                      onChange={e => etappeAktualisieren(idx,'unterkunft_typ',e.target.value)}
                      className="eingabe">
                  ...
                  <Feld label="Unterkunft-Name">
                    <input value={etappe.unterkunft_name}
                      onChange={e => etappeAktualisieren(idx,'unterkunft_name',e.target.value)}
                ...
                <Feld label="Tipps für andere Familien">
                  <textarea value={etappe.tipps}
                    onChange={e => etappeAktualisieren(idx,'tipps',e.target.value)}
          ...
          <div className="flex gap-3">
            <button onClick={() => setSchritt('basis')} className="btn-ghost flex-1">
              ← Zurück
            </button>
